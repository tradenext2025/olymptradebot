...
from config import TELEGRAM_TOKEN, ADMIN_EMAIL, ADMIN_PASSWORD
...
from auth import start_auto_refresh, selenium_login

logging.basicConfig(format="%(asctime)s - %(levelname)s - %(message)s", level=logging.INFO)

async def post_init(app):
    # Step 1 - Selenium auto login (gets fresh token)
    if ADMIN_EMAIL and ADMIN_PASSWORD:
        print(f"Auto logging in as {ADMIN_EMAIL}...")
        start_auto_refresh(ADMIN_EMAIL, ADMIN_PASSWORD)
        # Wait for first login
        await asyncio.sleep(20)
    else:
        print("No email/password set - using config token")

    # Step 2 - Start real price WebSocket
    start_websocket()

    # Step 3 - Start 24/7 signal engine
    start_engine()

    # Step 4 - Start auto signal sender
